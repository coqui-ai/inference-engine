...
CoquiModelPackage::CoquiModelPackage()
  : beam_width_(-1)
  , n_steps_(-1)
  , n_context_(-1)
  , n_features_(-1)
...
int
CoquiModelPackage::init(const char* model_path)
{
  return COQUI_ERR_OK;
}

char*
CoquiModelPackage::decode(const DecoderState& state) const
{
...
Metadata*
CoquiModelPackage::decode_metadata(const DecoderState& state,
                                   size_t num_results)
{
