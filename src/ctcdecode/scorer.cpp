...
int Scorer::load_lm(const std::string& lm_path)
{
  ...
  if (access(filename, R_OK) != 0) {
    return COQUI_ERR_SCORER_UNREADABLE;
  }
  ...
  if (!lm::ngram::RecognizeBinary(filename, model_type)) {
    return COQUI_ERR_SCORER_INVALID_LM;
  }
  ...
  if (package_size <= trie_offset) {
    ...
    return COQUI_ERR_SCORER_NO_TRIE;
  }
...
int Scorer::load_trie(std::ifstream& fin, const std::string& file_path)
{
  ...
  if (magic != MAGIC) {
    ...
    return COQUI_ERR_SCORER_INVALID_TRIE;
  }
  ...
  if (version != FILE_VERSION) {
    ...
    std::cerr << std::endl;
    return COQUI_ERR_SCORER_VERSION_MISMATCH;
  }
