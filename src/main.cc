...
// remove for now to simplify build&test
#define NO_SOX
#define NO_DIR
...
ds_result
LocalDsSTT(CoquiModelPackage* aCtx, const short* aBuffer, size_t aBufferSize,
           bool extended_output, bool json_output)
{
  ...
  if (extended_output) {
    Metadata *result = Coqui_SpeechToTextWithMetadata(aCtx, aBuffer, aBufferSize, 1);
    res.string = CandidateTranscriptToString(&result->transcripts[0]);
    Coqui_FreeMetadata(result);
  } else if (json_output) {
    Metadata *result = Coqui_SpeechToTextWithMetadata(aCtx, aBuffer, aBufferSize, json_candidate_transcripts);
    res.string = MetadataToJSON(result);
    Coqui_FreeMetadata(result);
  } else if (stream_size > 0) {
    CoquiStreamingState* ctx;
    int status = Coqui_CreateStream(aCtx, &ctx);
    ...
    while (off < aBufferSize) {
      ...
      Coqui_FeedAudioContent(ctx, aBuffer + off, cur);
      off += cur;
      prev = last;
      const char* partial = Coqui_IntermediateDecode(ctx);
      if (last == nullptr || strcmp(last, partial)) {
        printf("%s\n", partial);
        last = partial;
      } else {
        Coqui_FreeString((char *) partial);
      }
      if (prev != nullptr && prev != last) {
        Coqui_FreeString((char *) prev);
      }
    }
    if (last != nullptr) {
      Coqui_FreeString((char *) last);
    }
    res.string = Coqui_FinishStream(ctx);
    ...
    while (off < aBufferSize) {
      ...
      off += cur;
      prev = last;
      const Metadata* result = Coqui_IntermediateDecodeWithMetadata(ctx, 1);
      ...
      Coqui_FreeMetadata((Metadata *)result);
    }
    const Metadata* result = Coqui_FinishStreamWithMetadata(ctx, 1);
    res.string = CandidateTranscriptToString(&result->transcripts[0]);
    Coqui_FreeMetadata((Metadata *)result);
    free((char *) last);
  } else {
    res.string = Coqui_SpeechToText(aCtx, aBuffer, aBufferSize);
  }
...
#if !defined(NO_SOX) && TARGET_OS_OSX
...
void
ProcessFile(CoquiModelPackage* context, const char* path, bool show_times)
{
  ds_audio_buffer audio = GetAudioBuffer(path, Coqui_GetModelSampleRate(context));
  ...
  if (result.string) {
    printf("%s\n", result.string);
    Coqui_FreeString((char*)result.string);
  }
...
main(int argc, char **argv)
{
  ...
  // Initialise STT
  CoquiEngine* engine;
  int status = Coqui_InitEngine(&engine);
  if (status != COQUI_ERR_OK) {
    char* error = Coqui_ErrorCodeToErrorMessage(status);
    fprintf(stderr, "Could not init engine: %s\n", error);
    free(error);
    return 1;
  }

  CoquiModelPackage* ctx;
  // sphinx-doc: c_ref_model_start
  status = Coqui_LoadModelPackage(engine, model, &ctx);
  if (status != COQUI_ERR_OK) {
    char* error = Coqui_ErrorCodeToErrorMessage(status);
  ...
  if (set_beamwidth) {
    status = Coqui_SetModelBeamWidth(ctx, beam_width);
    if (status != COQUI_ERR_OK) {
  ...
  if (scorer) {
    status = Coqui_EnableExternalScorer(ctx, scorer);
    if (status != COQUI_ERR_OK) {
      fprintf(stderr, "Could not enable external scorer.\n");
      return 1;
    }
    if (set_alphabeta) {
      status = Coqui_SetScorerAlphaBeta(ctx, lm_alpha, lm_beta);
      if (status != COQUI_ERR_OK) {
  ...
  if (hot_words) {
    ...
    for ( std::string hot_word_ : hot_words_ ) {
      ...
      status = Coqui_AddHotWord(ctx, word, boost);
      if (status != COQUI_ERR_OK || !boost_is_valid) {
